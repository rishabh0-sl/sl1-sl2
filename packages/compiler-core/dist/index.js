...
export class LLMCompiler {
    ...
    async validateSelectorsWithMCP(url, scenarios) {
        ...
        const improvedScenarios = await Promise.all(scenarios.map(async (scenario) => {
    ...
    async executeScenarioWithMCP(url, steps) {
    ...
    async executeStepWithMCP(step, url, isFirstStep) {
    ...
    async generateGeminiScenarios(input) {
        ...
        try {
            ...
            const geminiResponse = await this.geminiService.generateTestScenarios(input.objective, input.url, input.credentials);
