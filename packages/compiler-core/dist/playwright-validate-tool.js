export class PlaywrightValidateSelectorTool {
    constructor() {
        ...
        this.inputSchema = {
            ...
            properties: {
                ...
                context: {
                    type: 'string',
                    description: 'Context for validation (e.g., "validation", "improvement")',
                    enum: ['validation', 'improvement', 'testing']
                },
                page: {
                    type: 'object',
                    description: 'Playwright page object (will be provided by MCP server)'
                }
            },
            required: ['selector']
        };
    ...
    async execute(args) {
        const { selector, context = 'validation', page } = args;
        if (!page) {
            return {
                isValid: false,
    ...
    async suggestAlternativeSelectors(originalSelector, page) {
        try {
            ...
            return {
                isValid: false,
                error: `Failed to suggest alternatives: ${error instanceof Error ? error.message : 'Unknown error'}`
            };
        }
    }
...
export const playwrightValidateSelectorTool = new PlaywrightValidateSelectorTool();
